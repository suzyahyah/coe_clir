...
sstage=0
estage=0
translate=0 # Translate must be done in stage0
reset=0 # copies all directories
query_english_only=1 # query

processd=(doc query bitext rel)
#processd=(doc)
...
if [ $reset -eq 1 ]; then
  ...
  printf "Copying TRECDIR from $TRECDIR_org to $TRECDIR.."
...
if [ $sstage -le 0 ] && [ $estage -ge 0 ]; then
  ...
  #######################################################
    ...
    # For this we use pre-trained fairseq with at least cuda10
    # Note this requires rtx gpus on COE-Grid
    # 1. Reserve GPU/Submit job
    # 2. module load cuda10.0/toolkit
  ...
  for lang in "${!L[@]}"; do
    ...
    if [[ "${processd[@]}" =~ "bitext" ]]; then
      ...
      if [ $lang == "chinese" ]; then
        ...
        printf "Tokenizing for chinese..\n"
        python src/preprocess.py --mode "tokenize" --docdir ${bitext2}_tm
        cp ${bitext2}_tm.temp ${bitext2}_tm
      fi
    fi
    ...
    if [[ "${processd[@]}" =~ "doc" ]]; then
      ...
      #  rm -r ${src_docs}_tm
      #  cp ${src_docs}_tm.temp ${src_docs}_tm
      #fi
...
if [ $sstage -le 7 ] && [ $estage -ge 7 ]; then
  ...
  for lang in "${!L[@]}"; do
    ...
    for qtype in all title; do
      ...
      tmmap=results/TREC/$lang/tm.$qtype.map
      tmrank=results/TREC/$lang/tm.$qtype.ranking
