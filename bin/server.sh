#!/usr/bin/env bash
# Credits: Taken from Sun Shuo github repo
# Convenience script for managing ElasticSearch server
version=7.5.0
ESDIR="elasticsearch-$version"
...
#    ES_JAVA_OPTS="-Xms${es_memory} -Xmx${es_memory}" ./external_tools/elasticsearch-${version}/bin/elasticsearch -d -Ehttp.port=${es_port}
    ES_JAVA_OPTS="-Xms${es_memory} -Xmx${es_memory}" $ESDIR/bin/elasticsearch -d -Ehttp.port=${es_port}
